...
class ControllerNode(Node):
    def __init__(self):
        ...
        self.cmd_vel_pub = self.create_publisher(Twist, '/skypal/cmd_vel', 10)
        ...
        self.get_logger().info('Skypal Controller Node Initialized. Publishing heartbeat at 50Hz.')
    ...
    def joy_callback(self, msg):
        ...
        if len(self.last_buttons) > 0:
            ...
            if len(msg.buttons) > 9 and msg.buttons[9] == 1 and self.last_buttons[9] == 0:
                self.get_logger().info("Options pressed: Sending 'arm_offboard' command")
                sys_msg = String()
            ...
            if len(msg.buttons) > 8 and msg.buttons[8] == 1 and self.last_buttons[8] == 0:
                self.get_logger().info("Share pressed: Sending 'land' command")
                sys_msg = String()
                sys_msg.data = "land"
                self.sys_cmd_pub.publish(sys_msg)
        ...
        if len(msg.axes) >= 5:
            ...
            twist.linear.x = msg.axes[4] * max_speed  # Right Stick U/D -> Pitch Forward
            twist.linear.y = msg.axes[3] * max_speed  # Right Stick L/R -> Roll Left
            twist.linear.z = msg.axes[1] * max_speed  # Left Stick U/D -> Throttle Up
            twist.angular.z = msg.axes[0] * max_speed # Left Stick L/R -> Yaw Left
